...
def readKlineFile(fn):
    df = pd.read_csv(fn).iloc[:, [0,2,3,4,5]]#在这里直接剔除code 这一列，后面不再处理了
#    df.columns=['date', 'code', 'OPEN', 'HIGH', 'LOW', 'CLOSE']
#    df = pd.read_csv(fn).iloc[:, :6]#在这里直接剔除code 这一列，后面不再处理了
...
#    dfKlines2['date'] = dfKlines['date'].apply( lambda x: date2num(datetime.datetime.strptime(x, '%Y-%m-%d')))
#    dfKlines2 = dfKlines.iloc[:, [0,2,3,4,5]] #剔除证券代码列，只保留日期、开高低收5列
    dfKlines2 = dfKlines

    lst = np.array(dfKlines2)
    lst[:,0] = range( len(lst))
    ...
    plt.savefig(r'C:\pv\【中金固收·可转债】走势切割与Python实践V2\%s %s %s.png'%(code, start_date, end_date), dpi=200 )
    plt.close()
...
if __name__ == '__main__':    
    ...
    end_date = '2021-08-01'        
    stock_lis = getfn(r'C:\pv\baostock data\stock20210722anxin')
    for fn in stock_lis:
        try:
...
#    fn = stock_lis[0]    
#    lp = LineProfiler()
#    lp.add_function(generatePunc)
#    lp_wrapper = lp(main)
#    lp_wrapper(fn, start_date, end_date)
#    lp.print_stats()
#    
